...
async function edit_element (element) {
   ...
   switch (target) {
      ...
      case "pier":   item = get_pier_by_id(id);
                       ...
                       $("#doctor_hospital").text(item.port);
                       prepare_ports_for_dropdown(target);
                       break;
...
function delete_element (item) {
   ...
   switch (target) {
      ...
      case "piers":
         message = "Ви дійсно хочете видалити інформацію про цю пристань";
         button = "Видалити";
         break;

      case "ships":
         message = "Ви дійсно хочете видалити інформацію про цей корабель";
         button = "Видалити";
         break;
   }
...
function display_piers_data (data) {
   ...
   for (let element of data) {
     ...
     `<tr>
         ...
         <td class="fit"> <span class="m-2">${element.shipCount}</span> </td>
         <td>${element.port}</td>
         <td>${get_icon_code()}</td>
      </tr>`;
...
function display_ships_data (data) {
   ...
   for (let element of data) {
     ...
     `<tr>
         <td> <span class="m-2">${element.id}</span> </td>
         <td>${element.name}</td>
         <td class="fit"> <span class="m-2">${element.unicID}</span> </td>
         <td>${element.pier}</td>
         <td>${element.port}</td>
         <td>${get_icon_code()}</td>
      </tr>`;
...
function modal_update_ports (added_new, id) {
   ...
   if (added_new) { add_port(name, address);      }
   else           { edit_port(id, name, address); }
...
function modal_update_piers (added_new, id) {
   ...
   let number    = $("#doctor_name").val();
   let shipCount      = $("#doctor_age").val();
   let port = $("#pier_port").text();

   port = port === "Виберіть порт" ? "Не встановлено" : port;

   if (added_new) { add_pier(number, shipCount, port, id);      }
   else           { edit_pier(number, shipCount, port, id); }
...
function modal_update_ships (added_new, id) {
   ...
   let unicID      = $("#ship_id").val();
   let pier   = $("#ship_pier").text();
   let port = $("#ship_port").text();

   pier   = pier   === "Виберіть пристань"  ? "Не призначено"  : pier;
   port = port === "Виберіть порт" ? "Не встановлено" : port;

   if (added_new) { add_ship(name, unicID, pier, port, id);      }
   else           { edit_ship(name, unicID, pier, port, id); }
      ...
      if (result.length != 0) {
         ...
         for (let item of result) {
            ...
            if (item.port === port) {
               ...
               list.append(`<li><span class="dropdown-item" ` +
                           `onclick="set_pier(this)">${item.number}</span></li>`);
            }
         }
      }
...
function clear_input() {
   ...
   switch (target) {
      ...
      case "ports": $("#hospital_name").val("");
                        $("#hospital_address").val("");
                        break;
      case "piers":   $("#doctor_name").val("");
                        $("#doctor_age").val("");
                        $("#doctor_hospital").text("Виберіть порт");
